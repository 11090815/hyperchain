...
func New(keyStore bccsp.KeyStore, curve *mathlib.Curve, translator crypto.Translator, exportable bool) (*csp, error) {
	base, err := NewImpl(keyStore)
	if err != nil {
		return nil, fmt.Errorf("failed instantiating base bccsp: [%s]", err.Error())
	}
...
func (csp *csp) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	// Validate arguments
	if k == nil {
		return nil, errors.New("invalid Key,i t must not be nil")
	}
