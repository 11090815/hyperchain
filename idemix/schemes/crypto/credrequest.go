...
// (from user to issuer)
func (i *Idemix) NewCredRequest(sk *mathlib.Zr, IssuerNonce []byte, ipk *IssuerPublicKey, rng io.Reader, tr Translator) (*CredRequest, error) {
	return newCredRequest(sk, IssuerNonce, ipk, rng, i.Curve, tr)
}

func newCredRequest(sk *mathlib.Zr, IssuerNonce []byte, ipk *IssuerPublicKey, rng io.Reader, curve *mathlib.Curve, tr Translator) (*CredRequest, error) {
	// Set Nym as h_{sk}^{sk}
	...
	// 3 elements of G1 each taking 2*mathlib.FieldBytes+1 bytes
	// hash of the issuer public key of length mathlib.FieldBytes
	// issuer nonce of length mathlib.FieldBytes
...
func (m *CredRequest) Check(ipk *IssuerPublicKey, curve *mathlib.Curve, tr Translator) error {
