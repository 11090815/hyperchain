...
func (i *Idemix) NewNymSignature(sk *mathlib.Zr, Nym *mathlib.G1, RNym *mathlib.Zr, ipk *IssuerPublicKey, msg []byte, rng io.Reader, tr Translator) (*NymSignature, error) {
	return newNymSignature(sk, Nym, RNym, ipk, msg, rng, i.Curve, tr)
}

func newNymSignature(sk *mathlib.Zr, Nym *mathlib.G1, RNym *mathlib.Zr, ipk *IssuerPublicKey, msg []byte, rng io.Reader, curve *mathlib.Curve, tr Translator) (*NymSignature, error) {
	// Validate inputs
	if sk == nil || Nym == nil || RNym == nil || ipk == nil || rng == nil {
		return nil, fmt.Errorf("cannot create NymSignature: received nil input")
	}
	...
	// - the signature label
	// - 2 elements of G1 each taking 2*mathlib.FieldBytes+1 bytes
...
func (sig *NymSignature) Ver(nym *mathlib.G1, ipk *IssuerPublicKey, msg []byte, curve *mathlib.Curve, tr Translator) error {
	...
	if !ProofC.Equals(curve.HashToZr(proofData)) {
		return fmt.Errorf("pseudonym signature invalid: zero-knowledge proof is invalid")
	}
