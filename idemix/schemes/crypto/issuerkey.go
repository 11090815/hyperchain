...
func newIssuerKey(AttributeNames []string, rng io.Reader, curve *mathlib.Curve, t Translator) (*IssuerKey, error) {
	// validate inputs
	...
	key.Ipk.HAttrs = make([]*translator.ECP, len(AttributeNames))
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal issuer public key: [%s]", err.Error())
	}
...
func (IPk *IssuerPublicKey) Check(curve *mathlib.Curve, t Translator) error {
	...
	HAttrs := make([]*mathlib.G1, len(IPk.GetHAttrs()))
...
func (IPk *IssuerPublicKey) SetHash(curve *mathlib.Curve) error {
	IPk.Hash = nil
	serializedIPk, err := proto.Marshal(IPk)
	if err != nil {
		return fmt.Errorf("failed to marshal issuer public key: [%s]", err.Error())
	}
