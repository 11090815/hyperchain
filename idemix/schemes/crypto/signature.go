...
) (*Signature, *bccsp.IdemixSignerMetadata, error) {
	switch sigType {
	case bccsp.Standard:
...
func prepare(
	...
	curve *mathlib.Curve,
	tr Translator,
) (*mathlib.G1, *mathlib.G1, *mathlib.G1, *mathlib.G1, *mathlib.G1, *mathlib.G1,
	[]byte,
	*mathlib.Zr,
	*mathlib.Zr,
	*mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr, *mathlib.Zr,
	[]*mathlib.Zr,
	nonRevokedProver,
	[]int, error,
) {
	...
	rAttrs := make([]*mathlib.Zr, len(HiddenIndices))
	...
	if err != nil {
		return nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, fmt.Errorf("failed to compute non-revoked proof: [%s]", err.Error())
	}

	// Step 1: First message (t-values)

	HAttrs := make([]*mathlib.G1, len(ipk.HAttrs))
...
func finalise(
	...
	tr Translator,
	t1, t2, t3 *mathlib.G1,
	APrime, ABar, BPrime *mathlib.G1,
	nonRevokedProofHashData []byte,
	E *mathlib.Zr,
	Nonce *mathlib.Zr,
	rSk, rSPrime, rR2, rR3, r2, r3, re, sPrime, rRNym *mathlib.Zr,
	rAttrs []*mathlib.Zr,
	...
	var Nym_eid *mathlib.G1
	var t4 *mathlib.G1
	var r_r_eid, r_eid *mathlib.Zr
	...
	// the signature label
	// 7 elements of G1 each taking 2*mathlib.FieldBytes+1 bytes
	// one bigint (hash of the issuer public key) of length mathlib.FieldBytes
	...
	if sigType == bccsp.EidNym {
		m = &bccsp.IdemixSignerMetadata{
			NymEIDAuditData: &bccsp.NymEIDAuditData{
				RNymEid: r_eid,
...
func (sig *Signature) AuditNymEid(
	...
	if err != nil {
		return fmt.Errorf("could not deserialize H_a_eid: [%s]", err.Error())
	}

	HRand, err := t.G1FromProto(ipk.HRand)
	if err != nil {
		return fmt.Errorf("could not deserialize HRand: [%s]", err.Error())
	}

	EidNym, err := t.G1FromProto(sig.EidNym.Nym)
	if err != nil {
		return fmt.Errorf("could not deserialize EidNym: [%s]", err.Error())
	}
...
func (sig *Signature) Ver(
	...
	if verType == bccsp.ExpectStandard && sig.EidNym != nil {
		return errors.New("EidNym available but ExpectStandard required")
	}

	verifyEIDNym := (verType == bccsp.BestEffort && sig.EidNym != nil) || verType == bccsp.ExpectEidNym
	...
	ProofSAttrs := make([]*mathlib.Zr, len(sig.GetProofSAttrs()))
	...
	if verifyEIDNym && meta != nil && meta.NymEIDAuditData != nil {
		...
		if !Nym_eid.Equals(EidNym) {
			return errors.New("signature invalid: nym eid validation failed")
		}
	}
