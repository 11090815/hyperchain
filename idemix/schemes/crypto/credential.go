...
type Translator interface {
	G1ToProto(*mathlib.G1) *translator.ECP
	G1FromProto(*translator.ECP) (*mathlib.G1, error)
	G1FromRawBytes([]byte) (*mathlib.G1, error)
	G2ToProto(*mathlib.G2) *translator.ECP2
	G2FromProto(*translator.ECP2) (*mathlib.G2, error)
}
...
func (i *Idemix) NewCredential(key *IssuerKey, m *CredRequest, attrs []*mathlib.Zr, rng io.Reader, t Translator) (*Credential, error) {
	return newCredential(key, m, attrs, rng, t, i.Curve)
}

func newCredential(key *IssuerKey, m *CredRequest, attrs []*mathlib.Zr, rng io.Reader, t Translator, curve *mathlib.Curve) (*Credential, error) {
	...
	if len(attrs) != len(key.Ipk.AttributeNames) {
		return nil, errors.New("incorrect number of attribute values passed")
	}
	...
	HAttrs := make([]*mathlib.G1, len(key.Ipk.HAttrs))
...
func (cred *Credential) Ver(sk *mathlib.Zr, ipk *IssuerPublicKey, curve *mathlib.Curve, t Translator) error {
	// Validate Input
	...
	if !B.Equals(BPrime) {
		return errors.New("b-value from credential does not match the attribute values")
	}
