...
type User interface {
	...
	NewKey() (*mathlib.Zr, error)

	// NewKeyFromBytes converts the passed bytes to a User secret key
	NewKeyFromBytes(raw []byte) (*mathlib.Zr, error)

	// MakeNym creates a new unlinkable pseudonym
	MakeNym(sk *mathlib.Zr, key IssuerPublicKey) (*mathlib.G1, *mathlib.Zr, error)

	NewNymFromBytes(raw []byte) (*mathlib.G1, *mathlib.Zr, error)

	// NewPublicNymFromBytes converts the passed bytes to a public nym
	NewPublicNymFromBytes(raw []byte) (*mathlib.G1, error)
}
...
type CredRequest interface {
	...
	// (from user to issuer)
	Sign(sk *mathlib.Zr, ipk IssuerPublicKey, nonce []byte) ([]byte, error)
...
type Credential interface {
	...
	Verify(sk *mathlib.Zr, ipk IssuerPublicKey, credential []byte, attributes []bccsp.IdemixAttribute) error
}
...
type SignatureScheme interface {
	...
	// If attributes[i].Type == schemes.IdemixHiddenAttribute then attribute i remains hidden and otherwise it is disclosed.
	// We require the revocation handle to remain undisclosed (i.e., attributes[rhIndex] == schemes.IdemixHiddenAttribute).
	...
	Sign(cred []byte, sk *mathlib.Zr, Nym *mathlib.G1, RNym *mathlib.Zr, ipk IssuerPublicKey, attributes []bccsp.IdemixAttribute,
	...
	// If attributes[i].Type == schemes.IdemixHiddenAttribute then attribute i remains hidden and otherwise
	...
	AuditNymEid(ipk IssuerPublicKey, eidIndex int, signature []byte,
		enrollmentID string, RNymEid *mathlib.Zr) error
}
...
type NymSignatureScheme interface {
	...
	Sign(sk *mathlib.Zr, Nym *mathlib.G1, RNym *mathlib.Zr, ipk IssuerPublicKey, digest []byte) ([]byte, error)

	// Verify verifies an idemix NymSignature
	Verify(pk IssuerPublicKey, Nym *mathlib.G1, signature, digest []byte) error
}
