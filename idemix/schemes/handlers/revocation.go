...
// false otherwise.
func (k *revocationSecretKey) IsPrivate() bool {
	return true
}
...
// Private returns true if this key is a private key,
// false otherwise.
func (k *revocationPublicKey) IsPrivate() bool {
	return false
}
...
func (i *RevocationPublicKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse revocation ECDSA public key bytes: [%s]", err.Error())
	}
