...
import pbmsp "github.com/11090815/hyperchain/protos-go/msp"
...
func getIdemixRoleFromMSPRole(role *pbmsp.MSPRole) int {
...
func getIdemixRoleFromMSPRoleType(rtype pbmsp.MSPRole_MSPRoleType) int {
...
func getIdemixRoleFromMSPRoleValue(role int) int {
	switch role {
	case int(pbmsp.MSPRole_ADMIN):
		return ADMIN.getValue()
	case int(pbmsp.MSPRole_CLIENT):
		return CLIENT.getValue()
	case int(pbmsp.MSPRole_MEMBER):
		return MEMBER.getValue()
	case int(pbmsp.MSPRole_PEER):
