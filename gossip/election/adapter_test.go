...
import (
	...
	"github.com/11090815/hyperchain/gossip/metrics/mocks"
...
func TestNewAdapter(t *testing.T) {
	...
	NewAdapter(mockGossip, selfNetworkMember.PKIid, []byte("channel0"),
		metrics.NewGossipMetrics(&disabled.Provider{}).ElectionMetrics, selfNetworkMember.ExternalEndpoint)
}
...
func TestAdapterImpl_Peers(t *testing.T) {
	...
	for id, adapter := range adapters {
		...
		for _, peer := range peers {
			if _, exist := myPeersOrg[peer.ExternalEndpoint]; !exist {
				t.Errorf("Peer %s PKID not found", peer.ExternalEndpoint)
			}
		}
	}
}
...
func TestAdapterImpl_Gossip(t *testing.T) {
	...
	channels := make(map[string]<-chan *pbgossip.GossipMessage)
	...
	for {
		...
		case msg := <-channels[fmt.Sprintf("Peer%d", 1)]:
			...
			} else if !bytes.Equal(msg.GetLeadershipMsg().PkiId, sender.selfPKIid) {
...
type mockAcceptor struct {
	ch       chan *pbgossip.GossipMessage
...
func (g *peerMockGossip) Accept(acceptor common.MessageAcceptor, passThrough bool) (<-chan *pbgossip.GossipMessage, <-chan protoext.ReceivedMessage) {
	ch := make(chan *pbgossip.GossipMessage, 100)
	g.acceptorLock.Lock()
...
func (g *peerMockGossip) Gossip(msg *pbgossip.GossipMessage) {
	g.clusterLock.RLock()
	if g.cluster == nil {
		g.clusterLock.RUnlock()
		return
...
func (g *peerMockGossip) putToAcceptors(msg *pbgossip.GossipMessage) {
...
func (g *peerMockGossip) IsInMyOrg(member discovery.NetworkMember) bool {
	...
	if memberOrg, exists = g.pki2org[member.ExternalEndpoint]; !exists {
		return false
	}
...
func createCluster(pki2org map[string]string, peers ...int) (*clusterOfPeers, map[string]*adapterImpl) {
	...
	for _, peer := range peers {
		...
		peerMember := &discovery.NetworkMember{
			Metadata:         []byte{},
			ExternalEndpoint: peerEndpoint,
			PKIid:            peerPKID,
		}

		mockGossip := newGossip(peerEndpoint, peerMember, pki2org)
		adapter := NewAdapter(mockGossip, peerMember.PKIid, []byte("channel0"),
			metrics.NewGossipMetrics(&disabled.Provider{}).ElectionMetrics, peerMember.ExternalEndpoint)
...
func TestReportMetrics(t *testing.T) {
	...
	adapter := NewAdapter(mockGossip, nil, []byte("channel0"), electionMetrics, "nil")

	adapter.ReportMetrics(true)

	require.Equal(t,
		[]string{"channel", hex.EncodeToString([]byte("channel0"))},
