...
func (ca *CA) NewIntermediateCA() (*CA, error) {
	certKeyPair, err := newCertKeyPair(true, false, ca.endorser, ca.x509Cert) // 中级 CA 也是 CA，所以 isCA 变量是 true
	if err != nil {
		return nil, err
	}
	return &CA{CertKeyPair: certKeyPair}, nil
...
func (ca *CA) NewClientCertKeyPair() (*CertKeyPair, error) {
	return newCertKeyPair(false, false, ca.endorser, ca.x509Cert)
}
...
func (ca *CA) NewServerCertKeyPair(hosts ...string) (*CertKeyPair, error) {
	return newCertKeyPair(false, true, ca.endorser, ca.x509Cert, hosts...)
}
